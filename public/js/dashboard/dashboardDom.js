...
$("body").on("click", ".btn-delete", async function (e) {
    ...
    if (section == "rappel-rdv") {
        ...
        if (deleted.success) {
            ...
            MessageAlert.create("information", "#input-message-alert", deleted.message).removeMessage(4000)
...
propositions.data.length > 0 ? propositions.data.map(proposition => {
    const html = setPropositionCard(proposition)
    $("#section-card-proposition").append(html)
}) : $("#section-card-proposition").append(`
    <p class="header-tag">
    ...
    Vous n'avez aucune proposition de rendez-vous en cours</p>
    `)
